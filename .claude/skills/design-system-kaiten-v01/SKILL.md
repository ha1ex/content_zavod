...
### Иллюстрации без подложки (ОБЯЗАТЕЛЬНО)

Иллюстрация, мокап или мини-превью интерфейса внутри карточки/ячейки лежит **прямо на фоне этой карточки**. Отдельную заливку под иллюстрацию не подкладываем — ни белую, ни серую, ни цветную.

- Не оборачивать иллюстрацию в контейнер с `background` / `bg-*`. Обёртка нужна только для раскладки (центрирование, `overflow:hidden`), но не для фона.
- Не добавлять под иллюстрацию отдельную рамку или «плашку» со своим скруглением. Если у самого мокапа есть рамка окна — она часть иллюстрации, её не дублируем.
- Правило действует на **всех брейкпоинтах** и не зависит от фона карточки: на белой карточке подложки нет, на серой — тоже нет.
- Причина: вторая заливка даёт «плашку в плашке», съедает воздух в карточке и конкурирует с самим мокапом за внимание.

```html
<!-- ❌ иллюстрация посажена на собственную заливку -->
<div class="mb-5 flex w-full justify-center rounded-xl bg-white p-3">…мокап…</div>

<!-- ✅ обёртка только для раскладки, фон — карточки -->
<div class="mb-5 flex w-full items-center justify-center overflow-hidden">…мокап…</div>
```

Тень у самой иллюстрации — **всегда без смещения: `X=0`, `Y=0`**. Направленную тень (`0 8px …`, `2px 3px …`) не используем: мокап не «висит» над карточкой, он лежит в её плоскости. Меняем только размытие и растяжение.

```css
/* ❌ */ box-shadow: 0 8px 18px -8px rgba(0,0,0,.28);
/* ✅ */ box-shadow: 0 0 18px -8px rgba(0,0,0,.28);
```

Проверяй на всех трёх брейкпоинтах: вокруг мокапа не должно читаться прямоугольника другого оттенка, чем фон карточки, и тень не должна уходить в одну сторону.
