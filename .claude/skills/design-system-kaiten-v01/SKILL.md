...
## Висячие предлоги (ОБЯЗАТЕЛЬНО — проверять всегда)

Всегда проверяй текст на висячие предлоги. Предлог (и другое короткое слово) не должен оставаться последним словом в строке — если слово, к которому относится предлог, не помещается в строку, предлог переносится на новую строку **вместе** со своим словом.

Реализация — неразрывный пробел `&nbsp;` между коротким словом и следующим за ним словом. Неразрывный пробел склеивает их, поэтому браузер при переносе уносит предлог на новую строку с его словом.

Привязывай через `&nbsp;` к следующему слову:

- Предлоги: `в`, `во`, `на`, `с`, `со`, `к`, `ко`, `по`, `за`, `до`, `от`, `из`, `у`, `о`, `об`, `для`, `при`, `над`, `под`, `про`, `без`, `через`.
- Союзы и частицы из 1–3 букв: `и`, `а`, `но`, `или`, `да`, `же`, `ли`, `бы`, `не`, `ни`.
- Перед короткими словами тоже: `это`, `как`, `что`, `так`.

Также неразрывным пробелом связывай: число и его единицу/слово (`5&nbsp;задач`, `до&nbsp;5&nbsp;пользователей`, `2026&nbsp;год`), инициалы и фамилию, `№&nbsp;5`, тире между словами оставляй на нижней строке (используй неразрывный пробел перед тире: `слово&nbsp;— слово`).

Примеры:

```html
<!-- ❌ предлог «в» висит в конце строки -->
<p>Управляйте задачами и процессами в одной платформе</p>

<!-- ✅ «в» уедет на новую строку вместе с «одной» -->
<p>Управляйте задачами и&nbsp;процессами в&nbsp;одной платформе</p>
```

Проверяй это перед сдачей любого макета на всех ширинах (desktop / tablet / mobile) — на узких экранах висячих предлогов больше всего. Правило применяется к заголовкам, подзаголовкам, body-тексту, кнопкам, подписям — ко всему тексту макета.
