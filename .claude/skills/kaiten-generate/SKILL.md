...
- **Отзывы/доверие → `ReviewSlider` ПО УМОЛЧАНИЮ.** Для блока «кейсы / доверие / отзывы» бери `ReviewSlider` (слайдер карточек: лого/название компании, цитата, аватар-инициал, имя+роль, опц. кнопка «Читать кейс»). `SocialProof` (статичная сетка), `TestimonialQuote` (одна крупная цитата), `LogoCloud` (полоса лого) — альтернативы: только если явно нужны/просят в ТЗ. Слот социального доказательства в любом layout закрывается любым из этих блоков (conformance их приравнивает). Правило: `reviews-default-slider`.
  - **Листалки — только при >3 отзывах.** `ReviewSlider` показывает стрелки перелистывания, лишь когда `reviews.length > 3`. При 3 и меньше карточки видны целиком, навигация скрыта. Поведение зашито в компоненте `Review.tsx`. Правило: `reviews-nav-when-4plus`.
