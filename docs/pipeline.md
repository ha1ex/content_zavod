...
`agent build` прогоняет brief через `routePipeline()` — детерминированный
гейт домена с двумя исходами:
...
> Исторически был третий исход — `legacy` (быстрый one-shot
> `prepare → write spec → apply` для простых брифов). Удалён в июне 2026:
> один конвейер сборки, фазированный контроль качества для всех брифов.
...
| `pageArchetype` | `saas` / `waitlist` / `enterprise` / `event` — управляет селекцией контекста |
...
prompt'ы фаз для хост-агента (Claude Code / Codex / ChatGPT с file access).
Хост-агент = LLM.
...
# 2. Entry point: гейт домена + phased pipeline. LLM-вызова НЕТ.
$ pnpm -w run harness agent build landing \
    --slug <slug> \
    --brief content/briefs/<slug>.json
```

Orchestrator идёт по фазам P0..P8. На каждой LLM-фазе он останавливается со
status `awaiting-host-agent` и пишет prompt в
`.context/pipeline/<slug>/p<N>-*.prompt.md`, где указано:
- задача фазы + контекст (артефакты предыдущих фаз),
- JSON Schema ожидаемого артефакта,
- `outputPath` — куда записать результат.

```bash
# 3. Хост-агент (Claude Code и т.п.) читает prompt фазы, пишет артефакт,
#    повторно запускает build/run — и так до P8. Полный LandingSpec
#    появляется в content/landings/<slug>.json на фазах P6/P7.
...
**Прямой flow с API-ключом** (`pnpm -w run harness generate landing`) сохранён как
...
фразу «сгенерируй лендинг по <brief>», сам зовёт `build` → заполняет артефакты фаз →
...
# 3. Сборка: гейт домена + phased pipeline (P0..P8)
pnpm -w run harness agent build landing \
  --slug <slug> --brief content/briefs/<slug>.json

# 4. Хост-агент (Claude Code) заполняет prompt'ы LLM-фаз из
#    .context/pipeline/<slug>/ и перезапускает build до зелёного P8.
