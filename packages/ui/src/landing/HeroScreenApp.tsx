...
/** Иконки продукта наружу: их берут другие блоки (напр. табы ViewSwitcher) как `kaiten:<имя>`. */
export const APP_ICONS = M;
      ...
      <div className="app__tree-list">
        {TREE.map((it, i) => (
          <div className={it.active ? 'app__tree-it is-active' : 'app__tree-it'} key={i}>
            ...
            <span className="app__tree-lbl">{it.label}</span>
            {it.active && (
...
/** Стили интерфейса наружу: их переиспользуют другие моки того же экрана. */
...
.hsi .app__blocker{display:flex;align-items:center;gap:8px;background:#e57373;color:#fff;border-radius:6px;padding:8px 5px 9px 10px;margin-bottom:-2px;font-size:13px;font-weight:700;line-height:18px;letter-spacing:0}
