  ...
  return (
    <section className="w-full">
      <div className="mx-auto w-full max-w-(--container-kaiten) px-4 py-12 md:px-6 md:py-16 xl:px-0 lg:py-24">
        ...
        <div
          className={cn(
            'grid w-full grid-cols-2 gap-x-4 gap-y-8 md:gap-10',
            withMock
              ? cn(
                  mockCenter
        ...
        >
                ...
                {g.links.map((l, li) => (
                  <li key={l.href + l.label}>
                    <a
                      ...
                      className="group -mx-2 flex items-center gap-3 rounded-(--radius-lg) px-2 py-1.5 text-sm text-(--color-text-primary) transition-colors hover:text-(--color-text-accent)"
                    >
                      {/* Иконки Landing-DS: серые, при наведении — фирменный фиолетовый вместе с подписью */}
                      <span className="inline-flex h-6 w-6 shrink-0 items-center justify-center text-[#bdbdbd] transition-colors group-hover:text-(--color-text-accent)">
          ...
          {withMock && (
            // Мок — только на десктопе: на планшете и мобилке он мелкий и нечитаемый
            <div data-comp="link_groups.centerMockVariant" className={cn("order-first mx-auto hidden w-full min-w-0 max-w-[560px] md:col-span-2 xl:col-span-1 xl:block xl:max-w-[470px] xl:self-center", mockCenter ? 'xl:order-2' : 'xl:order-1')}>
