...
export interface MediaCopyProps {
   ...
   */
  mediaTight?: boolean;
  /** Скругление картинки меньше: 8px вместо 12/16. Opt-in. */
  mediaRadiusSmall?: boolean;
...
function MediaCopyVisual({
  ...
  border?: boolean;
  noRound?: boolean;
  /** Скругление меньше: 8px вместо 12/16. Opt-in. */
  radiusSmall?: boolean;
  frameWhite?: boolean;
  fill?: boolean;
  round?: boolean;
  ...
  if (src) {
    return (
      <img
        ...
        alt={alt ?? ''}
        loading="lazy"
        className={cn('block h-auto w-full', !noRound && (radiusSmall ? 'rounded-(--radius-lg)' : 'rounded-(--radius-xl) lg:rounded-(--radius-2xl)'), grayShadow && 'shadow-[0_0_40px_rgba(45,45,45,0.12)]', border && 'border border-(--color-border-default)', frameWhite && 'border-[6px] border-white bg-white')}
      />
    );
  }
