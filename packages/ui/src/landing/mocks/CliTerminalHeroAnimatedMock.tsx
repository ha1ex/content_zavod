 ...
 * Слева — терминал, где посимвольно печатается мини-сессия; справа — карточка
 * задачи Kaiten (визуальный язык из WindowCardMock: заголовок, метаданные,
 * тулбар с пиллом «→ В работе», блок «Основные параметры» со строками
 * Расположение/Участники/Метки), которая заполняется под каждую команду.
 * Первая команда подключает профиль — после неё появляется карточка, дальше
 * данные добавляются как обычно. Цикл (CSS @keyframes, 15s):
 *   1. `profile add work --domain acme --token ***`  → из скелетона появляется карточка (#123, Доска / Очередь);
 *   2. `members add … "Анна Петрова"`                 → в поле «Участники» появляется Анна Петрова;
...
.cth .c1{ animation:cthT1 15s steps(49,end) infinite; }
...
@keyframes cthT1{ 0%,1%{width:0} 11%{width:49ch} 92%{width:49ch} 93%,100%{width:0} }
...
const MARKUP = `<div class="wrap" aria-hidden="true">
  <div class="term">
    ...
    <div class="body">
      <div class="line l1"><span class="p">$</span><span class="cmd c1"><span class="w">kaiten profile add work</span> <span class="fl">--domain</span> <span class="str">acme</span> <span class="fl">--token</span> <span class="str">***</span></span><span class="cur u1"></span></div>
