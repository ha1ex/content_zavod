...
/**
 * ClosedPerimeter — «закрытый контур компании»: сервер в центре, вокруг него
...
export type ClosedPerimeterProps = {
...
const DEFAULT_AVATARS: PerimeterAvatar[] = [
...
const DEFAULT_REQUESTS: PerimeterRequest[] = [
...
.clp .clp__fit{width:100%;overflow:hidden;aspect-ratio:1120/472}
.clp .clp__scene{transform-origin:top left;width:1120px}
@media(max-width:767px){
  .clp .clp__fit{aspect-ratio:auto;overflow:visible}
  /* !important гасит инлайновый масштаб: на мобилке сцена перестраивается, а не ужимается */
  .clp .clp__scene{width:auto!important;transform:none!important}
}
@media(prefers-reduced-motion:reduce){
  .clp .op-pulse,.clp .op-led{animation:none}
  .clp .op-pulse{opacity:0}
  .clp .op-led{opacity:.6}
}
.clp .dot{width:8px;height:8px;border-radius:50%}
.clp .op-contour{background:#F6F2FC;position:relative;width:1120px;max-width:100%;height:472px;margin:0 auto;border-radius:var(--radius-3xl,24px);
  background:rgba(239,233,249,.28)}
.clp .op-contour::before{content:"";position:absolute;inset:0;border-radius:var(--radius-3xl,24px);pointer-events:none;
...
.clp .op-contour{background:#F6F2FC;border:1px solid #EBE1F9}
.clp .op-contour::before{display:none}
.clp .op-lock{position:absolute;top:-16px;left:50%;transform:translateX(-50%);display:inline-flex;align-items:center;gap:6px;
...
.clp .op-lock svg{width:14px;height:14px}
.clp .op-links{position:absolute;inset:0;width:100%;height:100%;z-index:0;opacity:1}
.clp .op-links path{fill:none;stroke:rgba(125,76,207,.24);stroke-width:2;stroke-dasharray:5 6}
.clp .op-pulse{fill:url(#clpGrad);opacity:0;offset-rotate:auto}
.clp .p1{offset-path:path("M216 96 L476 96")}
.clp .p2{offset-path:path("M216 235 L340 235 Q350 235 350 225 L350 156 Q350 146 360 146 L476 146")}
.clp .p3{offset-path:path("M216 374 L410 374 Q420 374 420 364 L420 206 Q420 196 430 196 L476 196")}
.clp .p4{offset-path:path("M904 96 L644 96")}
.clp .p5{offset-path:path("M904 235 L780 235 Q770 235 770 225 L770 156 Q770 146 760 146 L644 146")}
.clp .p6{offset-path:path("M904 374 L710 374 Q700 374 700 364 L700 206 Q700 196 690 196 L644 196")}
.clp .p7{offset-path:path("M560 340 L560 238")}
.clp .op-pulse{animation:clpPulse 3.6s cubic-bezier(.4,0,.6,1) infinite}
.clp .op-server{position:absolute;left:50%;top:48px;transform:translateX(-50%);width:196px;z-index:2;
...
.clp .op-server__head{display:flex;align-items:center;justify-content:center;gap:8px;font-size:var(--fs-sm,14px);font-weight:400;margin-bottom:var(--sp-3,12px)}
.clp .op-unit{display:flex;align-items:center;gap:5px;background:var(--k100,#f5f5f5);border:1px solid var(--k200,#eee);border-radius:var(--radius-lg,8px);padding:8px 10px;margin-bottom:6px}
.clp .op-led{width:7px;height:7px;border-radius:50%}
.clp .led-g{background:#B49BE6;color:#B49BE6}
.clp .op-led{opacity:.35;animation:clpLed 2.4s cubic-bezier(.4,0,.2,1) infinite}
.clp .op-unit .op-led:nth-child(1){animation-delay:var(--row,0s)}
.clp .op-unit .op-led:nth-child(2){animation-delay:calc(var(--row,0s) + .25s)}
.clp .op-unit .op-led:nth-child(3){animation-delay:calc(var(--row,0s) + .5s)}
.clp .op-unit + .op-unit{--row:.12s}
.clp .op-unit + .op-unit + .op-unit{--row:.24s}
.clp .op-vent{flex:1;height:4px;border-radius:2px;background:var(--k300,#e0e0e0)}
.clp .op-vent:first-of-type{margin-left:6px}
.clp .op-unit:last-of-type{margin-bottom:0}
.clp .op-server__foot{font-size:11px;color:var(--text-secondary,#757575);text-align:center;margin-top:var(--sp-4,16px)}
.clp .op-card{position:absolute;z-index:1;background:#fff;border:1px solid var(--k200,#eee);border-radius:var(--radius-xl,12px);
  padding:var(--sp-3,12px);width:190px;height:104px;display:flex;flex-direction:column;justify-content:flex-start;
  box-shadow:none}
.clp .op-card > :last-child{margin-top:auto;margin-bottom:auto}
.clp .op-card__t + *{margin-top:0}
.clp .op-card__t{font-size:var(--fs-sm,14px);font-weight:400;margin-bottom:8px;display:flex;align-items:center;gap:6px}
.clp .op-ci{width:18px;height:18px;flex:none}
.clp .op-card--team{left:40px;top:48px}
.clp .op-card--tasks{left:40px;top:187px}
.clp .op-card--proj{left:40px;top:326px}
.clp .op-card--docs{right:40px;top:48px}
.clp .op-card--sd{right:40px;top:187px}
.clp .op-card--sales{right:40px;top:326px}
.clp .op-card--chat{left:50%;transform:translateX(-50%);top:326px}
.clp .op-kan{display:flex;gap:5px}
.clp .op-kcol{flex:1;display:flex;flex-direction:column;gap:4px}
.clp .op-kcol + .op-kcol{border-left:1px solid var(--k200,#eee);padding-left:6px}
.clp .kc{height:16px;border-radius:4px}
.clp .kc-v{background:#F0F0F2}
.clp .kc-o{background:var(--orange-12,#fff3e0)}
.clp .kc-g{background:#F0F0F2}
.clp .kc-b{background:var(--green-12,#e9f5ea)}
.clp .kc-n{background:#FDE8E6}
.clp .kc-g2{background:var(--orange-12,#fff3e0)}
.clp .op-avs{display:flex;align-items:center}
.clp .op-avs .av{width:24px;height:24px;border-radius:50%;border:2px solid #fff;margin-left:-6px;display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:600;color:#fff}
.clp .op-avs .av:first-child{margin-left:0}
.clp .op-avs .av--plus{background:var(--k200,#eee);color:var(--text-secondary,#757575);border:2px solid #fff}
.clp .op-gantt{position:relative;height:44px}
.clp .op-gantt i{position:absolute;height:6px;border-radius:3px;top:1px}
.clp .op-docx{display:flex;gap:8px}
.clp .op-docx__side{width:46px;flex:none;display:flex;flex-direction:column;gap:5px;border-right:1px solid var(--k200,#eee);padding-right:7px}
.clp .dxr{display:flex;align-items:center;gap:3px}
.clp .dxr .dot{width:5px;height:5px;border-radius:2px;background:var(--k300,#e0e0e0);flex:none}
.clp .dxr .ln{flex:1;height:5px;border-radius:3px;background:var(--k200,#eee)}
.clp .op-docx__page{flex:1;min-width:0;display:flex;flex-direction:column;gap:5px}
.clp .dx-t{height:6px;width:62%;border-radius:3px;background:var(--k300,#e0e0e0)}
.clp .dx-callout{background:var(--blue-12,#e4f2fd);border-radius:4px;padding:4px 5px;display:flex;flex-direction:column;gap:3px}
.clp .dx-callout i{display:block;height:4px;border-radius:2px;background:#c4ddf5}
.clp .dx-l{height:5px;border-radius:3px;background:var(--k200,#eee)}
.clp .op-sd{display:flex;flex-direction:column;width:100%}
.clp .sd-row{display:flex;align-items:center;gap:5px;padding:5px 0;border-bottom:1px solid var(--k100,#f5f5f5)}
.clp .sd-row:first-child{padding-top:1px}
.clp .sd-row:last-child{border-bottom:0;padding-bottom:1px}
.clp .sd-row .d{width:5px;height:5px;border-radius:50%;flex:none}
.clp .sd-row .ln{height:5px;border-radius:3px;background:var(--k200,#eee)}
.clp .sd-row b{margin-left:auto;font-size:8px;font-weight:600;line-height:1;white-space:nowrap}
.clp .op-funnel{display:flex;flex-direction:column;gap:4px}
.clp .op-funnel i{height:8px;border-radius:4px;background:var(--brand-12,#efe9f9)}
.clp .op-funnel i:nth-child(2){background:var(--blue-12,#e4f2fd)}
.clp .op-funnel i:nth-child(3){background:var(--green-12,#e9f5ea)}
.clp .op-funnel b{font-size:10px;font-weight:var(--fw-reg,400);color:var(--text-title,#2d2d2d);margin-top:2px}
.clp .op-chat{display:flex;flex-direction:column;gap:6px}
.clp .bub-line{display:flex;align-items:flex-end;gap:6px}
.clp .bub-av{width:18px;height:18px;border-radius:50%;border:2px solid #fff;color:#fff;font-size:9px;font-weight:600;display:inline-flex;align-items:center;justify-content:center;flex:none}
.clp .bub-line .bub--in{width:60%}
.clp .bub{height:16px;border-radius:5px;position:relative}
.clp .bub::before, .clp .bub::after{content:"";position:absolute;height:3px;border-radius:2px}
.clp .bub::before{left:7px;right:7px;top:4px}
.clp .bub::after{left:7px;right:45%;top:9px}
.clp .bub--in::before, .clp .bub--in::after{background:#dedee1}
.clp .bub--out::before, .clp .bub--out::after{background:#d9c5f5}
.clp .bub--in{width:70%;background:var(--k100,#f5f5f5);border-top-left-radius:2px}
.clp .bub--out{width:56%;background:var(--brand-12,#efe9f9);align-self:flex-end;border-top-right-radius:2px}
@media(max-width:767px){
  .clp .op-contour{height:auto;padding:var(--sp-10,40px) var(--sp-4,16px) var(--sp-4,16px)}
  .clp .op-links{display:none}
  .clp .op-server{position:static;transform:none;margin:0 auto}
  .clp .op-card{position:static;width:auto;transform:none;animation:none !important}
  .clp .op-card--chat{transform:none}
  .clp .op-contour{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));grid-auto-rows:auto;gap:var(--sp-3,12px);align-items:start}
  .clp .op-server{grid-column:1/-1;width:min(320px,100%);margin-inline:auto}
  .clp .op-card{width:auto;height:104px;min-height:0}
  .clp .op-lock{top:-15px}
}
@media(max-width:767px){
}
@media(max-width:1279px){
  .clp .op-contour, .clp .op-card, .clp .op-server{border-radius:var(--radius-xl,12px)}
}
.clp .bub-line--out{justify-content:flex-end}
.clp .bub-line--out .bub--out{width:56%}
@media(max-width:767px){
  .clp .op-card, .clp .op-server{box-shadow:none}
}
@media(max-width:767px){
}
@media(max-width:767px){
  .clp .op-contour{grid-template-columns:repeat(2,minmax(0,1fr))}
  .clp .op-card{height:auto;aspect-ratio:190/104}
}
@media(min-width:560px) and (max-width:767px){
  .clp .op-contour{grid-template-columns:1fr 1fr}
}
@media(max-width:559px){
  .clp .op-card{width:100%;max-width:360px;margin-inline:auto}
}
@media(max-width:767px){
  .clp .op-contour{align-items:stretch;grid-auto-rows:auto}
  .clp .op-card{aspect-ratio:190/104;height:auto;min-height:100px;padding:10px}
  .clp .op-card__t{margin-bottom:6px;font-size:12px}
  .clp .op-ci{width:14px;height:14px}
  .clp .op-server{margin-bottom:var(--sp-2,8px);align-self:start}
}
.clp .op-gantt::before, .clp .op-gantt::after{content:"";position:absolute;top:0;bottom:0;width:1px;background:#e8e8ec;z-index:0}
.clp .op-gantt::before{left:33%}
.clp .op-gantt::after{left:66%}
.clp .op-gantt i{z-index:1}
@media(min-width:768px) and (max-width:1279px){
}
@media(max-width:767px){
  .clp .op-server__head span{white-space:nowrap}
}
.clp .op-kcol::before{content:"";display:block;height:4px;border-radius:2px;background:var(--k200,#eee);margin-bottom:2px}
.clp .op-kcol:nth-child(1)::before{width:70%}
.clp .op-kcol:nth-child(2)::before{width:60%}
.clp .op-kcol:nth-child(3)::before{width:78%}
.clp .op-kcol:nth-child(4)::before{width:56%}
.clp .op-links{will-change:transform}
@keyframes clpPulse{0%{offset-distance:0%;opacity:0}12%{opacity:1}72%{opacity:1}88%{offset-distance:100%;opacity:0}100%{offset-distance:100%;opacity:0}}
@keyframes clpLed{0%,100%{opacity:.5}
  45%{opacity:.95}
  70%{opacity:.65}}`;
...
export default function ClosedPerimeter({
  ...
  return (
    <div className="clp" aria-label={ariaLabel}>
      <style dangerouslySetInnerHTML={{ __html: css }} />
      <div className="clp__fit" ref={fitRef}>
        <div
          className="clp__scene"
        ...
        >
          <div className="op-contour">
            ...
            <svg className="op-links" viewBox="0 0 1120 472" preserveAspectRatio="none" aria-hidden>
              <defs>
                <linearGradient id="clpGrad" x1="0" y1="0" x2="1" y2="0">
