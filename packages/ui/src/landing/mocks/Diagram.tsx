...
export interface DiagramsProps {
  title?: string;
  subtitle?: string;
  /** Без шапки секции и внешних отступов — только сетка диаграмм (для вкладки «Отчеты» в моках). */
  headless?: boolean;
}

export const DIAGRAMS_STYLE = `
.diagrams-mock{
...
.diagrams-mock.is-headless{padding:0;}
...
/**
 * Те же шесть диаграмм по отдельности, в порядке сетки: сгорание, скорость
 * команды, время выполнения, накопительная, спектральная, пропускная
 * способность. Берет ReportChartMock, чтобы показать одну диаграмму в карточке фичи.
 */
export const DIAGRAM_CARDS = CHARTS.split('<div class="rept">')
  .slice(1)
  .map((chunk) => `<div class="rept">${chunk}`);
...
export function Diagrams({
  ...
  return (
    <section className={headless ? 'diagrams-mock is-headless' : 'diagrams-mock'} aria-label="Отчёты и диаграммы">
      <style dangerouslySetInnerHTML={{ __html: DIAGRAMS_STYLE }} />
      {!headless && (
