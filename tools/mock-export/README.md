...
## Гант ресурсное планирование

Отдельный мокап: сверху `ModuleGanttChartMock` без изменений, снизу панель
ресурсного планирования по макету Landing-DS (node 11891-81326) —
переключатель «Часы / Карточки», строки сотрудников, загрузка по дням,
выходные серым, перегрузка красным.

```bash
node tools/mock-export/gantt-resource.cjs   # нужен out/gantt.html из render.cjs
```

Итог — `design-system/presentation-v02/assets/mocks/gantt-resource.png`.
Сетка дней берется из самой диаграммы (колонка названий 300px, 28 ячеек
по 26px), поэтому загрузка стоит ровно под шкалой. Хвост «Реализации»
вырезается из готового кадра: полосы справа лежат во вложенных обертках
без своих координат, границы реза берутся по строкам левой колонки.
