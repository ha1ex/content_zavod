...
Три точки конверсии: форма в первом экране, кнопка-якорь после программы (`CtaBanner`), форма в финале. Отдельной кнопки в первом экране нет — там сразу форма.
...
| Слот | Компонент |
|---|---|
| Форма | `RegistrationForm` — имя\*, email\*, телефон, компания/роль, чекбокс 152-ФЗ |
