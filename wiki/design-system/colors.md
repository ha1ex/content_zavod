...
**Заголовки, подзаголовки и основной текст — только тёмные (ОБЯЗАТЕЛЬНО).** Всё это `text-primary` (`#2d2d2d`); осветлять до серого запрещено:

- заголовки секций и **подзаголовки/описания под H1 и H2** — `text-primary`
- основной текст: параграфы, описания, буллиты, текст карточек, ответы аккордеона, текст отзыва — `text-primary`
- `text-secondary` (`#757575`) — только действительно вторичное: подписи, метаданные, плейсхолдеры, роль автора отзыва, disabled
- `neutral.500` (`#9e9e9e`) — только сноски и дисклеймеры

Правило действует на **всех брейкпоинтах — desktop, tablet, mobile**. Цвет текста от брейкпоинта не зависит (в отличие от радиуса блока, см. `radius.md`): уменьшение кегля на планшете и мобилке не даёт права осветлять текст.

**В CTA-блоках весь текст блока — тёмный.** В final CTA, secondary CTA и любом блоке на `action-primary-soft` (`#efe9f9`) серый не используется вообще. Единственное исключение — лейбл **на залитой кнопке**: `text-inverse` на `action-primary` (это контрол, а не текст блока).

Контраст на белом: `#2d2d2d` ≈ 13.8:1; `#757575` ≈ 4.6:1 (проходит WCAG AA); `#9e9e9e` ≈ 2.7:1 (**не проходит AA** — поэтому только некритичный текст).
...
- ❌ Серый подзаголовок или описание под H1/H2. Только `text-primary`.
- ❌ Осветлять body «для воздуха». Воздух даётся межстрочным интервалом и отступами, а не понижением контраста.
- ❌ Серый поясняющий текст в CTA-блоке.
- ❌ `neutral.500` (`#9e9e9e`) на смысловом тексте (оффер, состав тарифа, условия) — не проходит WCAG AA.
