...
| `AutomationRuleTriggerMock` | `automation-rule-trigger` | Конструктор правила автоматизации, половина «Когда»: событие «Карточка перемещена или создана» + условие «Новое местоположение карточки» (доска, колонка, дорожка), кнопка «+ И ЕСЛИ» |
| `AutomationRuleActionMock` | `automation-rule-action` | Конструктор правила автоматизации, половина «Выполнить»: действие «Установить срок», временная зона, срок «через 2 дня после события», кнопка «+ ДОБАВИТЬ ДЕЙСТВИЕ» |
...
| `AutomationDeadlineMock` | `automation-deadline` | Контроль сроков автоматизацией: карточки с дедлайнами и действиями правила (напоминание за сутки, пересчёт при переносе срока, уведомление руководителю) + всплывающее напоминание |
| `AutomationChecklistDoneMock` | `automation-checklist-done` | Анимация: пункты чек-листа отмечаются по очереди, счётчик доходит до 3/3, карточка переезжает из «В работе» в «Готово» |
| `AutomationCardFlowMock` | `automation-card-flow` | Поток карточки по доске: «Очередь → В работе → Готово» со стрелками между колонками + пунктирная стрелка к родительской карточке, которая двигается следом |
