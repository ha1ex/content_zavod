...
| `IntegrationsHubMock` | `integrations-hub` | Карта интеграций: карточка Кайтена в центре и десять сервисов вокруг (Telegram, Slack, Tilda, Google Forms, Zapier, Outlook, Google календарь, Яндекс Диск, GitLab, GitHub), соединенных проводами. Холст 1440px, в узких слотах масштабируется. Для первого экрана и блоков про подключение сервисов |
...
| `WindowRuleTriggerMock` | `window-rule-trigger` | Конструктор правила автоматизации, половина «Когда»: событие «Карточка перемещена или создана» + условие «Новое местоположение карточки» (доска, колонка, дорожка), кнопка «+ И ЕСЛИ» |
| `WindowRuleActionMock` | `window-rule-action` | Конструктор правила автоматизации, половина «Выполнить»: действие «Установить срок», временная зона, срок «через 2 дня после события», кнопка «+ ДОБАВИТЬ ДЕЙСТВИЕ» |
| `AutomationRulesListMock` | `automation-rules-list` | Экран «Автоматизации» пространства: список правил «Когда → Выполнить» с досками и тумблерами включения |
| `WindowDeadlineMock` | `window-deadline` | Контроль сроков автоматизацией: карточки с дедлайнами и действиями правила (напоминание за сутки, пересчёт при переносе срока, уведомление руководителю) + всплывающее напоминание |
| `WindowChecklistDoneMock` | `window-checklist-done` | Анимация: пункты чек-листа отмечаются по очереди, счётчик доходит до 3/3, карточка переезжает из «В работе» в «Готово» |
| `WindowCardFlowMock` | `window-card-flow` | Поток карточки по доске: «Очередь → В работе → Готово» со стрелками между колонками + пунктирная стрелка к родительской карточке, которая двигается следом |
| `ReportsChartsCascadeMock` | `reports-charts-cascade` | Отчеты каскадом: окна «Накопительная диаграмма потока» и «Контрольный график» одного размера, второе смещено вправо и вниз, холст 592px |
| `WorkspaceViewMock` | `workspace-view-board` / `-list` / `-table` / `-timeline` / `-calendar` / `-reports` | Одно пространство «Запуск продукта / Маркетинг» в шести представлениях: доска с дорожками, список по колонкам, таблица, таймлайн по дням, календарь, отчеты. Во всех видах одни и те же карточки, в тулбаре подсвечен активный вид — для блока «виды пространства» |
| `WorkspaceCreateMock` | `window-workspace-create` | Семейство Window: то же окно создания пространства под именем window-*. Псевдоним `workspace-create` оставлен для существующих лендингов |
| `WorkspaceViewMock` (view=board) | `window-board-new` | Семейство Window: окно пространства с пустой «Новой доской» — колонки, дорожки «Срочно» и «Обычный приоритет» и открытое меню колонки. Псевдоним `workspace-view-board` оставлен |
| `WorkspaceCreateMock` | `workspace-create` | Создание пространства: на фоне приложение с деревом пространств и доской «Маркетинг», поверх выпадающее меню «Добавить» с подсвеченным пунктом «Пространство». 640px |
| `WorkspaceSpacesMock` | `workspace-spaces` / `workspace-access` | Пространства компании: дерево пространств и досок в боковом меню со уровнем доступа у каждого — и окно доступа к пространству (приглашение коллеги, роли участников, группа отдела) |
| `WindowProjectModalMock` | `window-project-modal` | Окно карточки проекта «Редизайн сайта»: блокировка, метки, срок, бюджет, чек-лист, дочерние карточки и панель комментариев — все данные и общение по задаче в одной карточке |
...
| `ModuleWorkspaceBoardMock` | `module-workspace-board` | Пространство с меню слева и доской «Запуск продукта» (доска первого экрана лендинга «Единое рабочее пространство»): дорожки, 4 колонки, анимация переноса карточки с плашкой-приемником. 1360px |
| `ModuleBoardsOriginMock` | `module-boards-origin` | Интерфейс Кайтена целиком, как в продукте: шапка, колонка разделов, «Дерево» с открытой доской, панель видов, доска «Задачи команды» на 4 колонки (метки, сроки, блокировка, чек-лист), колонка инструментов. Тот же интерфейс, что на первом экране лендинга «Единое рабочее пространство». 1920px |
| `ModuleBoards` (`ModuleBoardsMock`) | `module-boards` | Пространство «Проекты» с несколькими досками: портфель проектов, свёрнутые доски проектов и развёрнутая доска «Письма», 1360px |
| `ModuleScrumMock` | `scrum-board-wide` | Та же скрам-доска целиком, все шесть колонок, обрезана по высоте до пропорций окон WorkspaceViewMock (760×440) — для блока «виды пространства» |
| `ModuleScrumMock` | `scrum-board` | Фрагмент скрам-доски спринта: свимлейны по приоритету, колонки «Бэклог спринта» → «В работе» → «Ревью», карточки со стори-поинтами, тегами, чек-листами и сроками (показываются первые три колонки из шести) |
| `ModulePlatformKaiten` | `platform-kaiten` | Карта продукта Kaiten: «Ядро платформы» (Проекты / Задачи / Знания с мини-мокапами), «Инструменты и модули» (Service Desk, CRM, BPM, BI, Холст, Коммуникации, Интеграции), «Сквозной ИИ» |
| `ExcelGrowMock` | `excel-grow` | Таблица задач в Excel, которая растет без конца: строки едут вверх, кадр растворяется, в подвале множатся листы — визуальный аргумент «потолка таблицы» |
| `ExcelTableViewMock` | `excel-table-view` | Табличное представление Kaiten: ответственный, срок, статус и метка в строке, фильтры и сортировка сверху, из строки открывается карточка задачи |
| `TaskCardFullMock` | `task-card-full` | Карточка задачи Kaiten целиком: параметры, описание, чек-лист с прогрессом, файлы и панель обсуждения |
| `WindowCardMock` | `window-card` | Окно карточки задачи Kaiten: заголовок, тулбар действий и основные параметры (расположение, тип, участники, срок, метки, приоритет) |
| `WindowTicketModalMock` | `window-ticket-modal` | Окно обращения Service Desk: карточка с описанием, файлами и связями слева, комментарии команды справа |
| `WindowTicketModalOrigin` | `window-ticket-modal-origin` | Карточка задачи Кайтена один в один с продукта: поля, файлы, чек-лист и связи слева, комментарии команды справа |
| `CardChecklistRelationsMock` | `card-checklist-relations` | Фрагмент карточки 1:1: чек-лист с прогрессом и связи с дочерней карточкой |
| `PlatformSliderMock` | `platform-slider` | Окно Кайтена с боковым меню, разделы сменяют друг друга: задачи, проекты, поддержка, базы знаний, чат, встречи, CRM, ИИ агенты |
| `ModulesCollageMock` | `modules-collage` | Пять модулей платформы плитками: чаты, документы, дашборды, встречи, служба поддержки |
| `BoardSignalsMock` | `board-signals` | Доска Kaiten с подписанными сигналами: просроченный срок, блокировка, срочная задача + легенда сигналов |
| `ExcelImportMock` | `excel-import` | Импорт задач из XLS/CSV: строки файла слева, карточки на доске справа, строка перелетает в колонку «Очередь» |
| `TabletPairMock` | `tablet-pair` | Пара планшетов: приглушенный сторонний сервис на заднем плане и доска Кайтена на переднем — переезд в Кайтен (порт слайда T12 презентаций V02) |
...
| `LaptopBoardTreeMock` | `laptop-boards` | Ноутбук с канбан-доской производства и развёрнутым деревом разделов: цех, снабжение, технологи, ОТК, руководство, бухгалтерия, проектный офис. 880px |
| `TabletBoardTreeMock` | `tablet-boards` | Тот же экран, что у `laptop-boards` (доски производства и дерево разделов), в рамке планшета: ровный белый безель, камера, мягкая тень. 760px |
| `TabletImageMock` | `tablet-team-board` | Планшет со скриншотом доски «Задачи команды» (`/brand/kaiten-board-team-tasks.webp`, 2000×1331) вместо нарисованного экрана. 760px |
...
### Справочный центр (портал помощи)

Аудитория: руководители поддержки и клиентского сервиса, ИТ- и HR-руководители,
продуктовые команды с внешней документацией, административные службы. Лексика:
справочный центр, портал помощи, разделы и статьи, поиск по материалам,
рекомендованные статьи, заявка, сервис Службы поддержки, статус обращения,
история заявок, брендирование, собственный домен, шаблон писем, индексация,
новостная лента. Домен визуально уникален: это **внешний портал с шапкой
компании-клиента** (логотип, поиск, «Создать заявку»), а не рабочее пространство
трекера. Поэтому `support-board` / `request-card` (внутренние экраны агента) и
`docs-tree` / `kb-public` (внутренняя база знаний) сюда не подходят.

| Mock | Variant slug | Что показывает |
|---|---|---|
| `HelpCenterPortalMock` | `help-center-portal` | Главная страница портала: шапка с логотипом компании, поиском и кнопкой «Создать заявку», структура разделов слева, подборка важных материалов справа, плашка «Не нашли ответ на свой вопрос?». Сигнатурный визуал первого экрана |
| `HelpCenterSectionsMock` | `help-center-sections` | Структура материалов: дерево разделов (главная, инструкции со статьями, вопросы и ответы, новости компании) и превью открытой статьи |
| `HelpCenterRequestFormMock` | `help-center-request-form` | Форма «Новая заявка»: выбор сервиса Службы поддержки, подсказка сервиса, заполненные поля обращения, кнопка отправки |
| `HelpCenterSearchMock` | `help-center-search` | Поиск по порталу: строка запроса «как создать доску» и список найденных статей с разделом каждой, первый результат подсвечен |
| `HelpCenterSuggestedArticlesMock` | `help-center-suggested-articles` | Подсказка статей при создании заявки: плашка «Не нашли ответ», поле «Название» с недописанным вопросом и блок «Рекомендованные статьи» |
| `HelpCenterRequestCardMock` | `help-center-request-card` | Обращение из портала на доске команды: Очередь → В работе → Готово, в карточке сервис, тема, комментарии, метка, автор; свежая заявка помечена как новая |
| `HelpCenterRequestsMock` | `help-center-requests` | Раздел «Мои заявки»: поиск, фильтр по статусу, таблица обращений — номер, статус, тема, сервис, дата |
| `HelpCenterBrandingMock` | `help-center-branding` | Окно «Брендирование»: горизонтальный и квадратный логотип, основной цвет портала, переключатель названия в шапке |
| `HelpCenterDomainMock` | `help-center-domain` | Окно браузера с адресом help.company.ru и статьей «Как оформить заказ» под шапкой с логотипом клиента |
| `HelpCenterEmailMock` | `help-center-email` | Письмо пользователю по шаблону компании: логотип, номер заявки, описание обращения, кнопка «Перейти в заявку», сноска про ответ письмом |
| `HelpCenterTemplateMock` | `help-center-template` | Готовый шаблон Справочного центра: разделы с заготовками статей и напоминание о подключении сервисов Службы поддержки |
| `HelpCenterPortalMock` | `help-center-portal-compact` | Та же главная портала, но вполовину ниже: у карточек только иконка и название, плашки «Не нашли ответ» нет. Для узких слотов вроде правой колонки градиентного CTA |
| `HelpCenterSetupMock` | `help-center-setup` | Настройки пространства Kaiten: выделенный пункт «Справочный центр» с кнопкой «Создать» и строка модуля «Служба поддержки» с тумблером и условием по тарифам |

**Reference:** [`wiki/landings/help-center-reference.md`](../landings/help-center-reference.md).
