...
| `BoardCreateColumnMock` | `board-create-column` | Пустая доска «Задачи команды» (скриншот `/brand/kaiten-board-empty.webp`) и раскрытое меню колонки: «Создать карточку», «Создать колонку» со «Слева» и «Справа», «Добавить 2 подколонки». 2000px |
| `SpaceSwitchAnimatedMock` | `space-switch-animated` | Анимация переключения пространств по сценарию ролика kaiten.ru: курсор идет по дереву, подсветка переезжает, рабочая область сменяет доски трех пространств («Разработка», «HR-отдел», «SMM»). Цикл 12 секунд, 1920px |
| `SpaceCreateAnimatedMock` | `space-create-animated` | Анимация создания пространства по ролику kaiten.ru: курсор жмет «+» в «Дереве», окно «Новое пространство» с набором названия, затем окно «Новая доска» с шаблонами «Простая доска», «Доска с дорожками», «Скрам». Цикл 12 секунд, 1920px |
| `ColumnCreateAnimatedMock` | `column-create-animated` | Анимация создания колонки по ролику kaiten.ru: меню колонки, «Создать колонку» → «Справа», окно с набором названия «Согласование», новая колонка появляется на доске с дорожками «Срочно» и «Обычный приоритет». Цикл 12 секунд, 1920px |
| `PortfolioCardAnimatedMock` | `portfolio-card-animated` | Анимация по ролику kaiten.ru на доске «Портфель проектов» с дорожками: перенос карточки «Проект Б» в «Контроль», календарь срока на 16 января, окно карточки с блоком «Блокировка». Цикл 18 секунд, 1920px |
| `PortfolioNestedBoardsMock` | `portfolio-board-real` | Пространство с несколькими досками проектов: три свернуты в полоски со счетчиком, четвертая раскрыта — колонки, подколонки, лимит WIP, карточки с путем до родителя и сроком. 1920px |
| `AdminSpaceRealMock` | `admin-space-real` | «Рабочий кабинет руководителя» в интерфейсе Кайтена: доски «Стратегические проекты компании» и «Проекты ИТ-департамента» рядом, «Развитие сотрудников» под ними, карточки с периодами и «Срочно». 1920px |
